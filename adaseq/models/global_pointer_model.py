...
class GlobalPointerModel(Model):
    ...
    #         entities = [
    #             (start, end + 1, self.id_to_label[type_id]) # score_metrix[type_id, start, end]
    ...
    def decode(self, entity_scores: torch.Tensor) -> List[List[TypedSpan]]:  # noqa
        ...
        for score_matrix in entity_scores:
            seen_entities = {}
            for type_id, start, end in zip(*np.where(score_matrix > 0)):  # type: ignore
                score = score_matrix[type_id, start, end]
                # 如果该(start, end)对还没有被记录，或者当前分数更高，则更新
                if (start, end) not in seen_entities or score > seen_entities[(start, end)][0]: # and (end - start) < 5
                    seen_entities[(start, end)] = (score, type_id)
            
            entities = [
                (int(start), int(end) + 1, self.id_to_label[type_id]) 
                for (start, end), (score, type_id) in seen_entities.items()
            ]
            ...
            for i, (start1, end1, label1) in enumerate(entities):
                ...
                for j, (start2, end2, label2) in enumerate(entities):
                    if i != j:
                        # 检查实体 i 是否被实体 j 包含
                        if label1 == label2 and start2 <= start1 and end2 >= end1:
                            is_contained = True
                            break
                if not is_contained:
                    filtered_entities.append((start1, end1, label1))
            
            batch.append(filtered_entities)
            # batch.append(entities)
        return batch
